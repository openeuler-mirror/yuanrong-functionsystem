...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| COMMON-001 | common service metadata | Full service/function validation for names, kind, runtime, CPU/memory, env, layers, hook handlers | Parses structs/YAML; full validation not found | C++ `common/service_json/service_json.cpp`; Rust `common/utils/src/service_json.rs`, `function_proxy/src/instance_ctrl.rs` | Invalid metadata accepted or fails later | No | P1 | Port validation gates or add compatibility validator before metadata is used. |
...
| COMMON-003 | common SSL | Resolve cert paths, validate files, set LiteBus SSL env vars | SSL flags accepted; equivalent env init not found in inspected path | C++ `common/utils/ssl_config.cpp`; Rust `CommonConfig`, proxy ignored flags | SSL deployment may start without C++ TLS behavior | No | P1 | Search all modules for SSL init, then implement/test LiteBus env setup if absent. |
...
| COMMON-006 | common/config | C++ flags drive behavior | Many Rust flags are accepted/ignored for startup | Rust `common/utils/src/cli_compat.rs`; `function_proxy/src/config.rs` | Operator enables no-op flag | Parser gate only | P2 | Create per-flag behavior inventory: implemented, no-op accepted, unsupported. |
| PROXY-001 | function_proxy invoke | Copy `InvokeOptions.customTag` into `CallReq.createoptions` | Not copied in `invoke_to_call` | C++ `busproxy/invocation_handler/invocation_handler.cpp`; Rust `busproxy/invocation_handler.rs` | Runtime/proxy loses force invoke, route, billing, or custom hints | Not specifically | P1 | Add regression test and copy custom tags into Rust call request. |
...
| PROXY-007 | function_proxy flags | Tracing, metrics, traefik, runtime direct connection, memory/OOM/user-log/NPU flags drive behavior | `ProxyCppIgnored` accepts many as strings/no-op | Rust `function_proxy/src/config.rs` | False confidence from accepted flags | Parser only | P2 | Produce flag-by-flag matrix and close high-risk behavior flags first. |
