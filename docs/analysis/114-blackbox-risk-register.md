...
| R4 | Package is not byte-for-byte/minimal equivalent | Closed for black-box compatibility; byte-for-byte remains policy boundary | Low/Medium | `docs/analysis/119-r4-package-layout-closure.md`: C++ package has 160 entries, Rust 186; only C++-minus-Rust entry is `libyaml_tool.so`; Rust-only release-gate scripts removed; `opentelemetry-cpp` dir restored | Keep compatible-superset policy unless release owners require restoring the C++ `libyaml_tool.so` helper |
