...
For the complete SDK-side design (yuanrong/Python), see `yuanrong/docs/features/sandbox-rest-api.md`.
...
## Port forwarding route kind

Frontend 在内部 `network.portForwardings` 中传递 `routeKind=public|direct|tunnel`。SandboxExecutor 与 SandboxdExecutor 仅在写回 InstanceInfo 的 `portForward` metadata 时保留该字段；下发 runtime-launcher/sandboxd 的底层映射仍为 TCP NAT。canonical 形式为 `<routeKind>+<backendScheme>:H:C`，例如 `direct+http:H:C`、`tunnel+http:H:C`、`public+http:H:C`。
