...
impl PortManager {
    ...
    /// Returns ports held by runtimes that are no longer in `active_runtime_ids` and releases them.
    pub fn recycle_orphan_ports(&mut self, active_runtime_ids: &HashSet<String>) -> Vec<u16> {
        let orphans: Vec<String> = self
            .by_runtime
            .keys()
            .filter(|k| !active_runtime_ids.contains(*k))
            .cloned()
            .collect();
        let mut freed = Vec::new();
        for id in orphans {
            if let Some(p) = self.release(&id) {
                freed.push(p);
            }
        }
        freed
...
impl SharedPortManager {
    ...
    /// Release any allocations whose `runtime_id` is not in the active set (stale proxy state recovery).
    pub fn recycle_orphans(&self, active_runtime_ids: &HashSet<String>) -> Vec<u16> {
        self.inner.lock().recycle_orphan_ports(active_runtime_ids)
    }
    ...
    pub fn allocated_count(&self) -> usize {
        self.inner.lock().by_runtime.len()
    }
}
