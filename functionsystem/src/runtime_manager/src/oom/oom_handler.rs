...
/// Re-apply cgroup v2 `memory.max` from a limit in GiB (same formula as spawn-time isolate).
pub fn refresh_cgroup_memory_limit(cgroup: &Path, mem_gib: f64) -> std::io::Result<()> {
    super::cgroup::write_memory_max_from_gib(cgroup, mem_gib)
}
