...
// Fixed conf/state names instead of pid-suffixed ones: the actor mailbox keeps
// scans serial, so no two logrotate runs race on them, a crashed run leaves at
// most one residual file that the next run overwrites, and logrotate reuses
// the same state to keep its cross-run archive-chain bookkeeping.
const char ROTATE_CONF_NAME[] = "runtime-log-rotate.conf";
const char ROTATE_STATE_NAME[] = "runtime-log-rotate.state";
                                  ...
                                  std::string &stderrOut)
{
    ...
    // Mirror statusFuture's timeout: a child stuck after the kill escalation
    // (rare, but would otherwise block this actor's mailbox forever) leaves the
    // stderr pipe open, so cap the wait. The fd is dup'ed inside ReadPipeAsync
    // and closed by its reader on completion, so never close GetErr() here.
    auto stderrOption = errorFuture.Get(static_cast<uint64_t>(rotateTimeoutMs));
    stderrOut = stderrOption.IsSome() ? stderrOption.Get() : "";
...
void LogManagerActor::RotateOversizeLogs(const std::vector<std::string> &files)
{
    ...
    for (const auto &file : files) {
        ...
        // 6=(\.\d+)?; a plain active "{rid}.log" has 3..6 empty. Guard on
        // smatch.size() so a future pattern change cannot silently misread the
        // indices and feed archives into rotation.
        ...
        bool isActiveLog =
            (std::regex_match(file, matchResult, PYTHON_RUNTIME_LOG_REGEX_PATTERN_REGEX) &&
             matchResult.size() >= 7 && matchResult[3].str().empty() && matchResult[4].str().empty() &&
             matchResult[5].str().empty() && matchResult[6].str().empty()) ||
            (std::regex_match(file, matchResult, STD_BOOTSTRAP_LOG_REGEX_PATTERN_REGEX) &&
             matchResult.size() >= 3 && matchResult[2].str().empty()) ||
    ...
    auto confPath = litebus::os::Join(runtimeLogsPath_, ROTATE_CONF_NAME, '/');
    auto statePath = litebus::os::Join(runtimeLogsPath_, ROTATE_STATE_NAME, '/');
