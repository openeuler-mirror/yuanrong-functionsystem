...
use yr_runtime_manager::health_check::{classify_wait_status, runtime_exit_log_message};
...
fn std_error_log_is_used_when_backtrace_is_absent() {
    ...
    let msg = runtime_exit_log_message("runtime-1", "instance-1", 9, &root, "std", "node-a", &root)
        .unwrap();
...
fn rust_captured_stderr_is_used_when_cpp_std_log_is_absent() {
    ...
    let msg =
        runtime_exit_log_message("runtime-1", "instance-1", 11, &root, "std", "node-a", &root)
            .unwrap();
