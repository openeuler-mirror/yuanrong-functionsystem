...
#ifndef RUNTIME_MANAGER_EXECUTOR_CONCH_EXECUTOR_H
#define RUNTIME_MANAGER_EXECUTOR_CONCH_EXECUTOR_H
...
// ConchExecutor launches Conch microVM sandboxes through jiuwenbox's existing
// HTTP-over-UDS API (same socket /run/jiuwenbox/jiuwenbox.sock, same
// /api/v1/sandboxes endpoints). jiuwenbox routes to the conch backend via the
// request's top-level "sandbox_runtime":"conch" field. Structurally
// a near-copy of SupervisorExecutor: the UDS plumbing, exec/delete paths, and
// response parsing are identical; only CreateRequest (the conch policy body)
// and the EXECUTOR_TYPE constant differ.
class ConchExecutor : public Executor {
public:
    explicit ConchExecutor(const std::string &name, const litebus::AID &functionAgentAID);

    ~ConchExecutor() override = default;
...
private:
    ...
    litebus::Future<nlohmann::json> SendRequestToConch(const std::string &method, const std::string &path,
...
class ConchExecutorProxy : public ExecutorProxy {
public:
    explicit ConchExecutorProxy(const std::shared_ptr<ConchExecutor> &executor) : ExecutorProxy(executor) {};

    ~ConchExecutorProxy() override = default;
    ...
    {
        return litebus::Async(executor_->GetAID(), &ConchExecutor::StopAllSandboxes);
    }
};
