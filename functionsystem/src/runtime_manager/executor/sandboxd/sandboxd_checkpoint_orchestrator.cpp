...
litebus::Future<runtime::v1::SandboxCheckpointResponse> SandboxdCheckpointOrchestrator::DoCheckpoint(
    const std::shared_ptr<runtime::v1::SandboxCheckpointRequest> &req)
{
    ASSERT_IF_NULL(sandboxd_);
    auto resp = std::make_shared<runtime::v1::SandboxCheckpointResponse>();
    return sandboxd_
        ->CallAsyncX("Checkpoint", *req, resp.get(), &runtime::v1::SandboxService::Stub::AsyncCheckpoint)
        .Then([req, resp](const Status &status) -> litebus::Future<runtime::v1::SandboxCheckpointResponse> {
            if (status.IsOk()) {
                return *resp;
            }
            runtime::v1::SandboxCheckpointResponse err;
