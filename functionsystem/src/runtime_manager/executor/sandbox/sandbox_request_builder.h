...
/**
 * SandboxStartParams — all inputs required to build a gRPC StartRequest.
 *
...
struct SandboxStartParams {
    ...
    // When non-empty: populate StartRequest.ckpt_dir for checkpoint-based start
 ...
 *   - Build a StartRequest from SandboxStartParams. When checkpointID is
 *     non-empty, populate ckpt_dir/trace_id for checkpoint-based start.
...
public:
    ...
    ~SandboxRequestBuilder() = default;

    std::pair<Status, std::shared_ptr<runtime::v1::StartRequest>> Build(
