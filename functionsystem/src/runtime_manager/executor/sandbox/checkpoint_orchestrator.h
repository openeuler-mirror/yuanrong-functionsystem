...
/**
 * CheckpointOrchestrator — manages the complete checkpoint lifecycle.
 *
 * Responsibilities:
 *   - TakeSnapshot:         DoCheckpoint → upload/register → update state.
 *   - Restore preparation:  download → add-ref (caller uses DoStart for the
 *     actual restore via StartRequest with ckpt_dir).
 ...
 *   - Checkpoint entries in RuntimeStateManager are set before start and
 *     cleared inside ReleaseCheckpointRef — never left dangling.
 */
class CheckpointOrchestrator : public std::enable_shared_from_this<CheckpointOrchestrator> {
public:
    ...
    ~CheckpointOrchestrator() = default;
     ...
     * On failure: returns error Status — caller must NOT proceed with start.
     */
