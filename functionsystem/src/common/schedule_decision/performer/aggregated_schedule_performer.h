...
#ifndef AGGRAVATED_SCHEDULE_PERFORMER_H
#define AGGRAVATED_SCHEDULE_PERFORMER_H
...
struct SelectResult {
    ...
    std::shared_ptr<std::deque<ScheduleResult>> successSelectResult;
};

class AggregatedSchedulePerformer : public SchedulePerformer {
public:
    explicit AggregatedSchedulePerformer(const AllocateType &type) : SchedulePerformer(type){};
    ~AggregatedSchedulePerformer() override = default;
...
#endif  // AGGRAVATED_SCHEDULE_PERFORMER_H
