...
public:
    ...
    void EndAll(Response &rsp)
    {
        for (auto it = requestMatch_.begin(); it != requestMatch_.end();) {
            it->second.promise.SetValue(rsp);  // copy
            litebus::TimerTools::Cancel(it->second.waitResponseTimer);
            it = requestMatch_.erase(it);
        }
    }
