...
// T11-1: Build with empty checkpointID → returns StartRequest with no ckpt_dir
TEST_F(SandboxRequestBuilderTest, BuildWithEmptyCheckpointIdReturnsStartRequest)
{
    ...
    EXPECT_TRUE(startReq->ckpt_dir().empty());
}

// T11-2: Build with non-empty checkpointID → returns StartRequest with ckpt_dir set
TEST_F(SandboxRequestBuilderTest, BuildWithNonEmptyCheckpointIdReturnsStartRequestWithCkptDir)
{
    ...
    EXPECT_TRUE(status.IsOk());
    ASSERT_NE(startReq, nullptr);
    EXPECT_EQ(startReq->ckpt_dir(), "ckpt-abc");
    EXPECT_EQ(startReq->trace_id(), "test-runtime");
}

// T11-3: Build with empty checkpointID → StartRequest is non-null
TEST_F(SandboxRequestBuilderTest, BuildStartResultIsNonNull)
{
...
// T11-4: Build with non-empty checkpointID → StartRequest has ckpt_dir set
TEST_F(SandboxRequestBuilderTest, BuildWithCheckpointIdHasCkptDirSet)
{
    ...
    ASSERT_TRUE(status.IsOk());
    ASSERT_NE(startReq, nullptr);
    EXPECT_EQ(startReq->ckpt_dir(), "ckpt-xyz");
}
