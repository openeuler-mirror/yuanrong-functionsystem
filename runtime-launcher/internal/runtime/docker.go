...
// DockerRuntime implements ContainerRuntime using the Docker Engine SDK.
...
func (d *DockerRuntime) Delete(ctx context.Context, containerID string, timeoutSeconds int64) error {
	...
	if err := d.client.ContainerStop(ctx, containerID, stopOpts); err != nil {
		log.Printf("[docker] failed to stop container %s: %v; trying force delete", containerID[:12], err)
	}

	// Force-delete container.
	err := d.client.ContainerRemove(ctx, containerID, container.RemoveOptions{Force: true})
	if err != nil {
		return fmt.Errorf("failed to delete container %s: %w", containerID[:12], err)
	}
	log.Printf("[docker] container deleted: %s", containerID[:12])
	return nil
}
