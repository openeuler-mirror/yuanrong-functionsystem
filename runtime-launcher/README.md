...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--backend` | `docker` | runtime-launcher 内部容器引擎：`docker` 或 `podman`；服务接口始终为 `SandboxService` |
...
服务定义以 `../proto/posix/sandbox_api.proto` 为准，runtime-launcher 的 Go 代码由该文件生成：
...
`rl-client` 是配套的 gRPC 测试客户端，用于验证 SandboxService 服务。使用前需先启动服务端。
