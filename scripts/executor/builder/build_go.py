...
def build_cli(root_path):
    ...
    utils.sync_command(
        cmd=[
            "protoc",
            f"--proto_path={app_path}/proto",
            f"--go_out={app_path}/internal/pb",
            "--go_opt=paths=source_relative",
            "message.proto",
        ],
        cwd=app_path,
    )
    build_golang(app_path, "yr", main_path, output_path)
...
def build_meta_service(root_path):
    ...
    main_path = os.path.join(app_path, "cmd", "main.go")
    output_path = os.path.join(root_path, "functionsystem", "output", "bin")
    build_golang(app_path, "meta_service", main_path, output_path)


def build_golang(app_path, app_name, main_path, output_path, go_ldflags="-s -w", cgo_enabled=False):
    """
