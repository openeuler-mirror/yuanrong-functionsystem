...
def pipe_command(cmd: list[str], cwd: str = None, env: dict[str, str] = None):
    """
    ...
    """
    log.info(f"Executing command[{cmd[0]}] in {cwd if cwd else os.getcwd()}")
    proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=cwd, env=env)
